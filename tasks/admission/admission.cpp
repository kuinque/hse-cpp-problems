...
bool operator!=(const Date& left_date, const Date& right_date) {
    return (left_date.day != right_date.day || left_date.month != right_date.month || left_date.year != right_date.year);
}
...
AdmissionTable FillUniversities(const std::vector<University>& universities, const std::vector<Applicant>& applicants) {
    ...
    std::sort(applicants_copy.begin(), applicants_copy.end(), [&](Applicant& left_applicant, Applicant& right_applicant) {
        return (left_applicant > right_applicant);
    });
    std::vector<University> universities_copy = universities;
    sort(universities_copy.begin(), universities_copy.end(), [&](University left_university, University right_university) {
    ...
    for (Applicant& applicant : applicants_copy) {
        for (std::string& wished_university : applicant.wish_list) {
            if (universities_student_count[university_position[wished_university]] == universities[university_position[wished_university]].max_students) {
                continue;
            }
